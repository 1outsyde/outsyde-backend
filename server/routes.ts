...
export async function registerRoutes(
  ...
  app.get("/api/businesses", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to fetch businesses" });
    }
  });
  ...
  app.get("/api/businesses/:id", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to fetch business" });
    }
  });
  ...
  app.patch("/api/businesses/:id", async (req, res) => {
    ...
    try {
      ...
      const updated = await storage.updateBusiness(
        req.params.id,
        req.body
      );
      ...
      res
        .status(500)
        .json({ error: "Failed to update business" });
    }
  });
  ...
  app.get("/api/cities/:id", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to fetch city" });
    }
  });
  ...
  app.post("/api/v1/auth/customer/signup", async (req, res) => {
    try {
      ...
      const payload = { userId: user.id, isVendor: false };
  ...
  app.post("/api/v1/auth/login", async (req, res) => {
    try {
      ...
      let isValidPassword = false;
      if (isLegacyHash(user.password)) {
        isValidPassword = legacyVerifyPassword(
          data.password,
          user.password
        );
      } else {
  ...
  app.post("/api/v1/auth/refresh", async (req, res) => {
    try {
      ...
      const storedToken = await storage.validateRefreshToken(
        refreshToken
      );
      if (!storedToken) {
        return res.status(401).json({
  ...
  app.get(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        const { city, category, search, page = "1", pageSize = "20" } =
          req.query;
  ...
  // All photographer CRUD lives in its own router
